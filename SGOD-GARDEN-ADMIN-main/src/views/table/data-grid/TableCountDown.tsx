...
import countdownService from 'src/services/countdown/countdown.service'
...
// const formatCategory = (countdown: any) => {
//   return countdown.map(() => {
//     return
//   })
// }
    ...
    renderCell: (params: GridRenderCellParams) => {
      ...
      return (
        <Box sx={{ display: 'flex', alignItems: 'center' }}>
          <Box sx={{ display: 'flex', flexDirection: 'column' }}>
            <Typography noWrap variant='body2' sx={{ color: 'text.primary', fontWeight: 600 }}>
              {moment(row.time_countdown).format('DD/MM/YYYY')}
            </Typography>
          </Box>
        </Box>
      )
      ...
      return (
        <Box sx={{ display: 'flex', flexDirection: 'column' }}>
          <Typography
            ...
            variant='body2'
            sx={{
              color: row.is_show ? 'green' : 'red',
              fontWeight: 600
            }}
          >
            {row.is_show ? 'Khả dụng' : 'Không khả dụng'}
          </Typography>
        </Box>
      )
    }
...
const TableCountDown = () => {
  ...
  const { data: countdown } = useSWR('GET_COUNTDOWN', countdownService.getAll)
  ...
  return (
    <Card>
      <CardHeader title='Quản lý sự kiện' />
      <DataGrid
        autoHeight
        pagination
        // rows={rows}

        rows={countdown ? loadServerRows(paginationModel.page, countdown) : []}
        rowCount={countdown ? countdown.length : 0}
        columns={columns}
