...
const renderImages = (params: GridRenderCellParams) => {
  ...
  return (
    <CustomAvatar
      src={row.images.includes('http') ? row.images[0] : `${process.env.NEXT_PUBLIC_BASE_URL}/${row.images[0]}`}
...
const TableProduct = () => {
  ...
  const { data: product } = useSWR('GET_PRODUCT', productService.getAll)
  ...
  return (
    <Card>
      ...
      <DataGrid
        ...
        rows={product ? loadServerRows(paginationModel.page, product) : []}
        rowCount={product ? product.length : 0}
        columns={columns}
